class PokemonsController < ApplicationController
  ...
  def new
    @haspoke = Pokemon.all
    if @haspoke.find_by(name:params[:name])  
      puts "ja está cadastrado"
    else
      @pokemon = Pokemon.new
      @pokemon.number = PokeApi.get(pokemon:params[:name]).id
  ...
  def destroy
    @pokemons =Pokemon.all
    if @pokemons.find_by(name:params[:name])
       @pokemons.find_by(id:params[:name]).destroy
    end
    redirect_to root_path
    respond_to do |format|
